...
public:
  ...
  static const uint16_t MAX_WEIGHTS = 512; // number of weight bits
  ...
  uint8_t weight_bits[(MAX_WEIGHTS + 7) / 8];
  ...
  uint16_t numWeights() const { return total_bits; }

  void setWeightsFromBits(const uint8_t *bits) {
    for (uint16_t i=0;i<total_bits;++i) {
      uint8_t mask = 1 << (i & 7);
      if (bits[i]) weight_bits[i>>3] |= mask;
      else weight_bits[i>>3] &= ~mask;
    }
  }

  inline int8_t bitSign(uint16_t bit_index) const {
    return ((weight_bits[bit_index>>3] >> (bit_index & 7)) & 1) ? 1 : -1;
  }
    ...
    for (uint8_t l=0;l<layer_count;++l) {
      ...
      for (uint8_t j=0;j<out_dim;++j) {
        ...
        for (uint8_t i=0;i<in_dim;++i) {
          int8_t w = bitSign(in_off + j*in_dim + i);
          sum += w * cur_in[i];
        }
        if (L.recurrent) {
          for (uint8_t i=0;i<out_dim;++i) {
            int8_t w = bitSign(rec_off + j*out_dim + i);
            sum += w * L.state[i];
          }
        }
        if (l==layer_count-1) {
...
uint8_t prob[BinaryRNN::MAX_WEIGHTS]; // probability of bit=1 in [0,255]
uint8_t cur_bits[BinaryRNN::MAX_WEIGHTS];
uint8_t prev_bits[BinaryRNN::MAX_WEIGHTS];
